...
export default function ReportForm() {
  ...
  const request = useCallback(async () => {
    ...
    try {
      ...
      const { task: newTask } = await response.data;
      setTask(newTask);
    } catch (e) {
      console.log(e);
    } finally {
  ...
  const handleSubmit = (e) => {
    ...
    chipData.forEach((el) => formData.append('photos', ...el.files));
      ...
      <form onSubmit={handleSubmit}>
        <Stack direction="row" alignItems="center" justifyContent="left" flexWrap="wrap" spacing={3}>
          ...
          <label htmlFor="icon-button-file">
            <Input accept="image/*" id="icon-button-file" type="file" name="images" onChange={fileUploadHandler} multiple="multiple" />
